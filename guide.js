...
const GUIDE = [
  ...
  { tab: 9, t: '3D', what: 'The 3D geometry and mesh: the blade made from the 2D setup (its side profile extended across the web, the gap varying there as the inputs say) or read from an STL or STEP file of the blade, placed over the web with its lowest point at the gap, and a hexahedral mesh of the slurry under it and in the film beyond the edge, on a strip around one location or the full web width. The 3D solve comes next (not built yet).',
    uses: 'Made from the 2D setup: the 2D\'s blade (entry, exit face), the notch face length, the gap and its variation across the web. From a file: the file\'s units and axes (machine direction, up) and the inlet distance. Both: the region and the 3D mesh in the inputs bar, the film beyond the edge from the 1D.',
    read: 'The toolbar turns the view (3D, side, top, front), shows or hides the blade, slurry, web and mesh, and scales the heights (the gap is thin). Drag to turn, wheel to zoom, right-drag to pan. The pills say whether the blade covers the region and its underside is a single height over the web (an overhang cannot be meshed this way). The table compares the 1D and the 2D; its 3D column fills once the 3D is solved.' },
