...
const GUIDE = [
  ...
  { tab: 4, t: '2D CFD', what: '2D Navier–Stokes flow under the blade, over its exit face and into the free film, with the meniscus and its contact line solved together with the flow, at four positions across the web. Velocity, pressure, shear, viscosity and more, post-processed from the stored solutions.',
    ...
    read: 'Run solves the four locations (each in its own worker). The toolbar picks the location (or Compare, or Difference), the field, streamlines, vectors, contours; the zoom bar on the plot zooms and pans (wheel, drag). Click the colour bar for its scale. The bottom panel has flow metrics, probes, cut lines, across-web and profile charts, fibre results, convergence, the mesh study, saved cases, problems, messages, history and the method. "Out of date" marks results whose inputs changed since they were solved.' },
  { tab: 9, t: '3D', what: 'The flow in 3D on a strip across the web around one location. The blade is made from the 2D setup (its side profile extended across the web, the gap varying there as the inputs say) or read from an STL or STEP file of the blade and placed with its lowest point at the gap. Solve 3D first solves each station across the strip in 2D at its own gap and contact angle, then couples them in a 3D Navier–Stokes solve with the meniscus and its contact line, so the flow across the web, which the 1D and 2D cannot have, is in it. The full web width comes next (a coupled iterative solver).',
    ...
    read: 'Solve 3D (Ctrl+Enter) runs in the background; Esc stops it; the mesh settings show about how large and long a solve is. The view colours the flow by a field (speed, pressure, shear rate, viscosity, velocity across the web) on its outer faces: the strip\'s two sides are cuts through the flow; turn the blade off to see the top. Drag to turn, wheel to zoom, right-drag to pan. The charts: the film and the contact line across the strip (3D against each station\'s own 2D), the pressure on the blade\'s underside, the pressure along the blade at the middle. The table\'s 3D column is the strip\'s middle. "Out of date" marks a result whose inputs changed since.' },
...
const METHODS = [
  ...
  { t: 'Variation across the web', d: 'The local gap is the machine height less the fibre thickness, plus the blade waviness (a sine of the chosen wavelength) and the fibre thickness variation; the local contact angle varies by the wetting variation.',
